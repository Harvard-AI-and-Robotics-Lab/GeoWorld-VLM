...
SPLIT_FILE="${SPLIT_FILE:-${REPO_ROOT}/splits/data_split.json}"
EXP_NAME="${EXP_NAME:-gemma4_lingbot_single_image}"
GPUS="${GPUS:-0,1}"
...
python train_gemma4_lingbot_spatial.py \
  --gpu 0 --teacher_gpu 1 \
  --gemma_model_name "${GEMMA_MODEL}" \
  --lingbot_model_dir "${LINGBOT_MODEL}" \
  --lingbot_code_dir "${LINGBOT_CODE}" \
  --data_dir "${ADAPTVIS_DATA_DIR}" \
  --prompts_dir "${ADAPTVIS_PROMPTS_DIR}" \
